...
export default function DevPanel() {
  ...
  async function saveSidebarSettings(e) {
    ...
    try {
      ...
      alert('Configurações salvas! As mudanças já estão aplicadas.')
    } finally {
      setSaving(false)
    }
  }
  ...
  async function createAdmin(e) {
    ...
    try {
      ...
      // Usa service key para inserir (DevPanel não tem sessão Supabase)
      const { createClient } = await import('@supabase/supabase-js')
    ...
    <div style={{ minHeight: '100vh', background: '#f9fafb' }}>
      ...
      <div style={{ padding: '28px 32px', maxWidth: 700, margin: '0 auto' }}>
          ...
          <form onSubmit={saveSidebarSettings}>
            <div className="form-row">
              <div className="form-group">
                ...
                <input
                  ...
                  placeholder="Ex: CATHEDRAL"
                />
                <small style={{ color: '#6b7280', fontSize: 12, marginTop: 4, display: 'block' }}>
                  Deixe em branco para usar o padrão "CATHEDRAL".
                </small>
              </div>
              <div className="form-group">
                ...
                <input
                  ...
                  placeholder="Ex: Cathedral Vouchers"
                />
                <small style={{ color: '#6b7280', fontSize: 12, marginTop: 4, display: 'block' }}>
            ...
            <div className="form-group">
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <input
                  ...
                  placeholder="Ex: CATH"
                  style={{ maxWidth: 140, fontFamily: 'monospace', fontWeight: 700, letterSpacing: 2, textTransform: 'uppercase' }}
                />
              ...
              <small style={{ color: '#6b7280', fontSize: 12, marginTop: 6, display: 'block' }}>
                Máximo 6 caracteres. Apenas letras e números. Afeta apenas novos vouchers gerados.
              </small>
            </div>
                ...
                {SIDEBAR_COLORS.map(c => (
                  <div key={c.value} onClick={() => setSidebarColor(c.value)}
                    style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8,
                      padding: '6px 12px', borderRadius: 8, border: `2px solid ${sidebarColor === c.value ? '#e2b04a' : '#e5e7eb'}`,
            ...
            <div className="form-row">
              ...
              <div className="form-group">
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <input type="color" value={sidebarMenuColor.startsWith('rgba') ? '#aaaaaa' : sidebarMenuColor} onChange={e => setSidebarMenuColor(e.target.value)}
                    style={{ width: 44, height: 36, border: '1px solid #e5e7eb', borderRadius: 6, cursor: 'pointer', padding: 2 }} />
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ width: 180, background: sidebarColor, borderRadius: 10, padding: '16px 0', fontFamily: sidebarFont }}>
                <div style={{ padding: '0 16px 12px', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
                  <div style={{ color: sidebarNameColor, fontSize: 15, fontWeight: 800, letterSpacing: 1 }}>
                ...
                {['Dashboard', 'Clientes', 'Operadores'].map(item => (
                  <div key={item} style={{ padding: '9px 16px', color: 'rgba(255,255,255,0.6)', fontSize: 12, display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ fontSize: 13, color: sidebarMenuColor }}>◈</span><span style={{ color: sidebarMenuColor }}>{item}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <Section title="Administradores do sistema">
              ...
              <tbody>
                {admins.map(a => (
                  <tr key={a.id}>
                    ...
                    <td>
                      <span className={`badge ${a.is_active === false ? 'badge-red' : 'badge-green'}`}>
          ...
          {!showNewAdmin ? (
            <button className="btn btn-primary btn-sm" onClick={() => setShowNewAdmin(true)}>
              + Novo Administrador
            </button>
          ) : (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {clearItems.map(item => (
              <div key={item.key} style={{
                ...
                border: `1px solid ${item.key === 'all' ? '#fca5a5' : '#fecaca'}`,
                borderRadius: 8,
              }}>
                <div>
                  <div style={{ fontWeight: item.key === 'all' ? 700 : 500, fontSize: 14, color: item.key === 'all' ? '#dc2626' : undefined }}>
                    {item.label}
                  </div>
                  {confirmClear[item.key] && (
                    <div style={{ fontSize: 12, color: '#dc2626', marginTop: 2, fontWeight: 600 }}>
                      Tem certeza? Clique novamente para confirmar — irreversível!
                    </div>
                  )}
                </div>
                <button
                  ...
                  style={{
                    background: confirmClear[item.key] ? '#dc2626' : '#fee2e2',
                    color: confirmClear[item.key] ? '#fff' : '#dc2626',
                    border: '1px solid #fecaca',
                    fontWeight: confirmClear[item.key] ? 700 : 400,
                  }}
